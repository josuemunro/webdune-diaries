...
def find_render_output():
    """Find the rendered MP4 in the ticker-anim directory or renders/ subdirectory."""
    ...
    if os.path.isdir(renders_dir):
        mp4s = sorted([f for f in os.listdir(renders_dir) if f.endswith(".mp4")])
        if mp4s:
            return os.path.join(renders_dir, mp4s[-1])
    return None
